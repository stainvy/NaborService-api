...
describe('EventRegisterWorker', () => {
  ...
  it('should successfully register participant and emit event', async () => {
    mockManager.findOne
      .mockResolvedValueOnce({ id: 'evt-1', status: EventStatusEnum.OPEN, maxParticipants: 10 })
      .mockResolvedValueOnce(null);
    mockManager.count.mockResolvedValue(5);
    mockManager.create.mockReturnValue({ eventId: 'evt-1', userId: 'usr-1' });

    await worker.process({ data: { eventId: 'evt-1', userId: 'usr-1' } } as any);

    expect(mockManager.save).toHaveBeenCalled();
    expect(mockEventsGateway.emitParticipantAdded).toHaveBeenCalledWith('evt-1', 'usr-1');
  });
  ...
  it('should throw UnrecoverableError if event is full', async () => {
    ...
    await expect(worker.process({ data: { eventId: 'evt-1', userId: 'usr-2' } } as any))
      .rejects.toThrow(UnrecoverableError);
    
    expect(mockEventsGateway.emitRegistrationFailed).toHaveBeenCalledWith('evt-1', 'usr-2', 'EVENT_FULL');
  });

  it('should throw UnrecoverableError if already registered', async () => {
    mockManager.findOne
      .mockResolvedValueOnce({ id: 'evt-1', status: EventStatusEnum.OPEN, maxParticipants: 10 })
      .mockResolvedValueOnce({ eventId: 'evt-1', userId: 'usr-1', status: ParticipantStatusEnum.REGISTERED });
    mockManager.count.mockResolvedValue(5);

    await expect(worker.process({ data: { eventId: 'evt-1', userId: 'usr-1' } } as any))
      .rejects.toThrow(UnrecoverableError);
  });
});
