...
describe('Neo4jSyncWorker', () => {
  ...
  it('should dispatch upsert-user correctly', async () => {
    const job = {
      data: { operation: 'upsert-user', data: { pgId: '123' } },
    } as any;
    await worker.process(job);
    expect(mockNeo4jSyncService.upsertUser).toHaveBeenCalledWith({
      pgId: '123',
    });
  });
  ...
  it('should dispatch upsert-listing correctly', async () => {
    ...
    const job = {
      data: {
        operation: 'upsert-listing',
        data: { pgId: '456', createdAt: now.toISOString() },
      },
    } as any;
    await worker.process(job);
    expect(mockNeo4jSyncService.upsertListing).toHaveBeenCalledWith(
      expect.objectContaining({ pgId: '456' }),
    );
  });

  it('should throw UnrecoverableError for non-transient errors', async () => {
    mockNeo4jSyncService.upsertUser.mockRejectedValue(
  ...
  it('should re-throw transient errors', async () => {
    mockNeo4jSyncService.upsertUser.mockRejectedValue(
      new Error('ECONNREFUSED 127.0.0.1:7687'),
    );
    const job = {
      data: { operation: 'upsert-user', data: { pgId: '123' } },
    } as any;
