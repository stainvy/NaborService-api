  ...
  async fetchWithRetry(
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        if (attempt >= maxAttempts) {
          if (error.name === 'AbortError') {
            throw new HttpRequestTimeoutException(
              `HTTP request to ${url} timed out after ${maxAttempts} attempts`,
            );
          }
          throw error;
        }
        ...
        const delay =
          backoffs[attempt - 1] || backoffs[backoffs.length - 1] || 1000;
        this.logger.warn(
          `Attempt ${attempt} failed for ${url}: ${error.message}. Retrying in ${delay}ms...`,
        );
