...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    ...
    try {
      ...
      if (userId) {
        ...
        this.redisClient.setex(`presence:${userId}`, 3600, 'online').catch(() => {});
      } else {
        client.disconnect();
      }
    } catch {
  ...
  emitRegistrationResult(userId: string, eventId: string, status: string) {
    this.notifyUser(userId, 'event:registration_result', { event_id: eventId, status });
  }

  emitRegistrationFailed(eventId: string, userId: string, reason: string) {
    this.notifyUser(userId, 'event:registration_failed', { event_id: eventId, reason });
  }

  emitParticipantAdded(eventId: string, userId: string) {
    if (this.server) {
      this.server.to(`event:${eventId}`).emit('event:participant_added', { event_id: eventId, user_id: userId });
    }
  }

  emitPlaceAvailable(eventId: string) {
    if (this.server) {
      this.server.to(`event:${eventId}`).emit('event:place_available', { event_id: eventId });
    }
  }

  emitWaitlistPromoted(userId: string, eventId: string) {
    this.notifyUser(userId, 'event:waitlist_promoted', { event_id: eventId, user_id: userId });
  }
