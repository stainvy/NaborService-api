...
import { EventTicket, EventTicketDocument } from '../../database/mongo-schemas/schemas/event-ticket.schema';
...
export class EventTicketService {
  ...
  private generateHmac(eventId: string, userId: string, firstName: string, customValue: string | null): string {
  ...
  async generateTicket(eventId: string, userId: string) {
    ...
    const existingTicket = await this.ticketModel.findOne({ pg_event_id: eventId, pg_user_id: userId });
    if (existingTicket) {
  ...
  async getTicketStream(eventId: string, userId: string) {
    const ticket = await this.ticketModel.findOne({ pg_event_id: eventId, pg_user_id: userId });
    if (!ticket) {
  ...
  async scanTicket(eventId: string, hmac: string, scannerId: string) {
    ...
    const ticket = await this.ticketModel.findOne({ 'qr_payload.hmac_sha256': hmac, pg_event_id: eventId });
    if (!ticket) {
    ...
    return { success: true, scannedAt: ticket.scanned_at, user: { id: ticket.pg_user_id, name: ticket.qr_payload.first_name } };
  }
}
