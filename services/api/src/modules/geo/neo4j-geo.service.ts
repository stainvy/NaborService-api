...
export class Neo4jGeoService {
  ...
  async assignNeighbourhood(lat: number, lng: number): Promise<NeighbourhoodAssignment | null> {
    ...
    const fallbackQuery = `
      ...
      LIMIT 1
    `;
    const fallbackResult = await this.neo4jService.run(fallbackQuery, { lat, lng });
  ...
  async computeGeoScore(userNeighbourhoodId: string, targetNeighbourhoodId: string): Promise<number> {
    ...
    `;
    const result = await this.neo4jService.run(query, { userNbId: userNeighbourhoodId, targetNbId: targetNeighbourhoodId });
  ...
  async findProximityEntities(userNeighbourhoodId: string, maxResults: number = 50): Promise<ProximityResult[]> {
    const checkQuery = `MATCH (n:Neighbourhood {pg_id: $userNbId}) RETURN n.pg_id AS id`;
    const checkResult = await this.neo4jService.run(checkQuery, { userNbId: userNeighbourhoodId });
    ...
    const result = await this.neo4jService.run(query, { userNbId: userNeighbourhoodId, maxResults });
    return result.records.map(record => ({
      entityPgId: record.get('entityPgId'),
      entityType: record.get('entityType').toLowerCase() as any,
  ...
  async createNeighbourhood(polygon: GeoJSON.Polygon, metadata: NeighbourhoodMetadata): Promise<NeighbourhoodWithAdjacencies> {
    ...
    await this.neo4jService.run(createQuery, {
      ...
      lat, lng, geometry: geomStr, area: areaM2
    });
    
    return await this.updateAdjacencies(metadata.pg_id, polygon, lat, lng, areaM2);
  }

  async updateNeighbourhoodPolygon(pgId: string, polygon: GeoJSON.Polygon): Promise<NeighbourhoodWithAdjacencies> {
    this.validatePolygon(polygon);
    ...
    await this.neo4jService.run(`MATCH (n:Neighbourhood {pg_id: $pgId})-[r:ADJACENT_TO]-() DELETE r`, { pgId });
  ...
  private validatePolygon(polygon: any) {
    ...
    for (const ring of polygon.coordinates) {
      if (!Array.isArray(ring) || ring.length < 4 || ring.length > 1000) {
        throw new BadRequestException('Polygon rings must have between 4 and 1000 positions');
      }
      const first = ring[0];
  ...
  private async updateAdjacencies(pgId: string, polygon: GeoJSON.Polygon, lat: number, lng: number, areaM2: number): Promise<NeighbourhoodWithAdjacencies> {
    ...
    for (const record of result.records) {
      ...
      try {
        ...
        if (otherGeom.type !== 'Polygon' && otherGeom.type !== 'MultiPolygon') continue;
