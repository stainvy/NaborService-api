...
export class GeoPipelineProcessor {
  ...
  async processListingGeocode(listingId: string, address: string): Promise<void> {
  ...
  async deleteEntityGeocoding(nodeLabel: string, entityPgId: string): Promise<void> {
    ...
    this.logger.log(`Deleted all geographic relationships for ${nodeLabel} ${entityPgId} (RGPD compliance)`);
  }
  ...
  private async orchestrateGeocoding(
    ...
    while (true) {
      try {
        this.logger.debug(`Geocoding ${nodeLabel} ${entityPgId} at address: ${address}`);
        
        const geoResult = await this.banService.geocode(address);
        this.logger.debug(`BAN Result confidence: ${geoResult.confidence}`);

        const assignment = await this.neo4jGeoService.assignNeighbourhood(geoResult.latitude, geoResult.longitude);

        if (!assignment) {
          this.logger.warn(`No neighbourhood found for ${nodeLabel} ${entityPgId}. Removing existing geographic links.`);
          await this.updatePostgresNeighbourhood(nodeLabel, entityPgId, null);
          await this.removeExistingRelationship(nodeLabel, entityPgId, relationshipType);
          return;
        }

        this.logger.debug(`Assigned to neighbourhood ${assignment.neighbourhoodId} via ${assignment.method}`);
        await this.updatePostgresNeighbourhood(nodeLabel, entityPgId, assignment.neighbourhoodId);
        await this.upsertRelationship(nodeLabel, entityPgId, assignment.neighbourhoodId, relationshipType);
        
        return; // Success, exit retry loop
      } catch (error) {
        if (attempt < delays.length) {
          this.logger.warn(`Failed to process geocoding for ${nodeLabel} ${entityPgId} (Attempt ${attempt + 1}/${delays.length + 1}). Retrying in ${delays[attempt]}ms...`);
          await new Promise(resolve => setTimeout(resolve, delays[attempt]));
          attempt++;
          continue;
        }
        
        this.logger.error(`Failed to process geocoding for ${nodeLabel} ${entityPgId} after ${attempt + 1} attempts: ${error.message}`, error.stack);
        throw error;
      }
    }
  }
  ...
  private async removeExistingRelationship(nodeLabel: string, entityPgId: string, relationshipType: string): Promise<void> {
    const query = `
  ...
  private async upsertRelationship(nodeLabel: string, entityPgId: string, neighbourhoodId: string, relationshipType: string): Promise<void> {
    const query = `
