import { Controller, Post, Patch, Param, Body, BadRequestException, UseGuards, HttpCode, HttpStatus } from '@nestjs/common';
...
export class NeighbourhoodAdminController {
  ...
  async triggerReconciliation(@Body('hours') hours?: number) {
    ...
    return { success: true, message: `Reconciliation triggered for the last ${lookback} hours.` };
  }
  ...
  async createNeighbourhood(
    ...
    return await this.neo4jGeoService.createNeighbourhood(body.polygon, body.metadata);
  }
  ...
  async updateNeighbourhoodPolygon(
    ...
    return await this.neo4jGeoService.updateNeighbourhoodPolygon(pgId, body.polygon);
  }
}
